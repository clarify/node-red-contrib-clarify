module.exports = function (RED) {
  ...
  function ClarifyEnsureSignalNode(config) {
    ...
    async function createSignal(node, data, context) {
      ...
      return node.api
        .ensureSignal(node.dataType, signal)
        .then(response => {
    ...
    function patchSignalItem(node, data, context) {
      ...
      if (checkName || checkLabels || checkEnums || checkEngUnit || checkLocations) {
        // Patch signal
        return node.api
          .metaQuery(`integrations/${node.api.integrationID}/signals/${node.signalID}`, 'PATCH', {}, data)
          .then(() => {
    ...
    this.on('input', async function (msg, send, done) {
      try {
        ...
        var signalEnumValues = RED.util.evaluateNodeProperty(
          node.signalEnumValues,
          node.signalEnumValuesType,
          node,
          msg,
        );
